...
Fixpoint filter'{X:Type} (f:X -> bool)(l:list X):list X :=
  match l with
    | nil => nil
    | h :: tl => if f h then h :: filter' f tl else filter' f tl
  end.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => (leb 7 x) && (evenb x)) l.
...
Theorem map_f_distr: forall (X Y:Type)(f:X->Y)(l r:list X),
...
Proof.
  ...
  induction l as [| n l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'. rewrite -> map_f_distr.
    reflexivity.
Qed. 
...
(* FILL IN HERE *)
(* 应该是有很多应用场景的比如X是一种wrap后的type,[Some 1, Some 2, None, Some 3]想要或者
里面所有数组的和 我们需要一个函数f:option ->nat -> nat这个时候就会有价值*)
(** [] *)
...
Proof. 
  unfold plus. unfold zero. unfold one. simpl.
  reflexivity.
Qed.
...
Print nat.
(*写出 这个函数的本质还是要理解这里nat的含义 nat = forall X : Type, (X -> X) -> X -> X
     : Type 用f作用在x上n次那么就是nat n的定义，我们把已经作用m次的数放在函数的位置上，那么这个
     函数的意思即使 用m作用n次 就是n * m *)
Example mult_1 : mult one one = one.
Proof. 
  unfold mult. unfold one.
  reflexivity.
Qed.
...
(* 将 m作用函数的每一层都映射为了n的函数*)



Check (zero (bool -> bool)).
