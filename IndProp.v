...
Proof.
 ...
 - intros E. induction E as [| |n' E' IHE'].
   ...
   + apply ev_sum. { apply IHIHE'. } { apply IHE1. }
 - intros E. induction E as [| n' E' IHE'].
   +  apply ev'_0.
   +  assert(H:S(S(n')) = n' + 2).
      { rewrite <- plus_1_l. rewrite plus_comm. rewrite <- plus_1_l. rewrite plus_comm. rewrite plus_assoc.
        assert(H1: 1 + 1 = 2). { reflexivity. } { rewrite H1. rewrite plus_comm. reflexivity. } }
      { rewrite H. apply ev'_sum. { apply IHE'. } { apply ev'_2. }
Qed.
(** [] *)
...
Proof.
  ...
  - apply IHEn'. simpl in Enm. apply evSS_ev in Enm. apply Enm.
Qed.
...
Theorem ev_n_n: forall n:nat, ev (n + n).
Proof.
  induction n as [|n' IHn'].
  - apply ev_0.
...
Proof.
  ...
  rewrite plus_assoc in Enp.
  assert(H: n + m + n = n +  n + m ).
  - rewrite <- plus_assoc. rewrite (plus_comm m n). rewrite plus_assoc. reflexivity.
  - rewrite H in Enp. rewrite <- plus_assoc in Enp. 
    assert(H': ev (n + n)).
    + apply (ev_n_n n).
    + apply (ev_ev__ev (n+n) (m + p) Enp ) in H'. apply H'.
Qed.
...
Inductive total_relation: nat -> nat -> Prop :=
  | tr1: forall n m, eq n m -> total_relation n m
  | tr2: forall n m, lt n m -> total_relation n m
  | tr3: forall n m, lt m n -> total_relation m n.
...
Proof.
  ...
  - assert(Hn_S: n <= S n).
    ...
    { apply (le_trans n (S n) m Hn_S) in H0. apply H0. }
Qed.
...
Proof.
 ...
 split.
 - induction n2.
    + rewrite <- plus_n_O in Hnm. apply Hnm.
    + apply IHn2. rewrite <- plus_n_Sm in Hnm. apply le_trans with (n:= S(S(n1 + n2))).
      { apply le_S. apply le_n. }
      { apply Hnm. }
 - induction n1.
   + simpl in Hnm. apply Hnm.
   + apply IHn1. simpl in Hnm. apply le_trans with(n:= S(S (n1 + n2))).
    { apply le_S. apply le_n. }
    { apply Hnm. }
Qed.
...
Theorem leq_f : forall n m:nat, leb n m = true <-> n <= m.
Proof.
  intros n m.
  split.
  - apply leb_complete.
...
Proof.
  intros n m o.
  rewrite leq_f. rewrite leq_f. rewrite leq_f.
  apply le_trans.
Qed.
...
(** - Which of the following propositions are provable?
      - [R 1 1 2]
      - [R 2 2 6]
      通过定义关系 可以去证明算法中的一些性质了 或者modelchecking中一些性质

    - If we dropped constructor [c5] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer. (m + n = o)

    - If we dropped constructor [c4] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer. yes
...
Theorem R_x_o_o: forall o, R 0 o o.
Proof.
...
Proof.
  ...
  - intros. generalize dependent n. generalize dependent o.
  induction m as [|m' IHm'].
    + induction n as [|n' IHn'].
      { generalize dependent o. induction o as [|o' IHo']. 
        { intros. apply c1. }
        { intros. inversion H. } }
      { induction o as [|o' IHo'].
        { intros. inversion H. } 
        { intros. apply c3. simpl in H. inversion H. apply R_x_o_o. } }
    + induction n as [|n' IHn'].
      { intros. simpl in H. unfold fR in H. Search plus. rewrite <- plus_n_O in H.
        rewrite H. apply c5. apply R_x_o_o. }
      { intros. unfold fR in H. simpl in H. rewrite <- plus_n_Sm in H. rewrite <- H.
        apply c3. apply c2.  apply IHm'. unfold fR. reflexivity. }
Qed.
...
Inductive subseq: list nat -> list nat -> Prop :=
  ...
  | s2: forall (x:nat) (xs ys:list nat), subseq xs ys -> subseq (x::xs) (x::ys)
  | s3: forall (x:nat) (xs ys:list nat), subseq xs ys -> subseq xs (x::ys).
...
Lemma Lapp: forall T (x:T) (xs:list T), x::xs = [x] ++ xs.
Proof.
  intros T x xs.
  induction xs.
  - simpl. reflexivity.
...
Proof.
  ...
  induction ss.
  - simpl. apply MStar0.
  - rewrite Lapp. apply MStarApp.
    + apply H. Search In. simpl. left. reflexivity.
