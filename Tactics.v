...
(** Briefly explain the difference between the tactics [apply] and
    ...
    applied?
(* apply的作用是根据 结论匹配 然后将前提加入证明的goal中，rewrite是重写某种程度上
可以替代apply但是感觉如果前提和结论没有关系的话 只能用apply*)
(* FILL IN HERE *)
*)
(** [] *)
...
Proof.
  ...
  - intros m H. destruct m as [|m'].
    ...
    + simpl in H. inversion H. rewrite <- plus_n_Sm in H1. rewrite <- plus_n_Sm in H1.
      ...
      rewrite plus_comm in H1. simpl in H1. inversion H1. symmetry in H2.
...
Theorem mult_distr_r : forall n m p:nat, (n + m) * p = n * p + m * p.
Proof.
   intros n m p.
   induction n as [|n' IHn'].
   - simpl. reflexivity.
...
Proof.
  ...
  induction n as [|n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite -> mult_distr_r. rewrite -> IHn'. reflexivity.
Qed.
...
Proof.
  ...
  -  simpl. destruct (split l') as [xs ys]. intros. inversion H. simpl.
     assert (IHuse: combine xs ys = l').
     +  apply IHl'. reflexivity.
     +  rewrite -> IHuse. reflexivity.
Qed.
...
Proof.
  ...
  induction n as [|n' IHn'].
  - intros p m H0 H1. inversion H0. destruct m.
    + apply H1.
    + inversion H2.
  - intros p m H0 H1. inversion H0. destruct m.
    + inversion H2. 
    + destruct p.
      { inversion H1. }
      { simpl. simpl in H0. simpl in H1. rewrite -> H0. apply IHn' with (m:=m).
        { apply H0. }
        { apply H1. }
      }
Qed.
