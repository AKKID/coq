...
Fixpoint count (v:nat) (s:bag) : nat :=
   match s with
    | nil => 0
    | h :: tl => if eqb h v then S (count v tl)
                  else count v tl
   end.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with
    | nil => nil
    | h :: tl => if eqb h v then tl
                  else h :: remove_one v tl
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
    | nil => nil
    | h :: tl => match eqb h v with
                  | true => remove_all v tl
                  | flase => h :: remove_all v tl
                 end
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
    | nil => true
    | h :: tl => match count h s2 with
                  | O => false
                  | _ => subset tl (remove_one h s2)
                 end
  end.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match l1, l2 with
    ...
    | _, nil => false
    | nil, _ => false
    | h1 :: l1', h2 :: l2' => if eqb h1 h2 then beq_natlist l1' l2'
                                  else false
  end.
...
Theorem eqb_n_n : forall n:nat, true = eqb n n.
Proof.
...
(** Prove that the [rev] function is injective -- that is,
    ...
    Theorem rev_i : forall (l1 l2:natlist), l1 = l2 -> rev l1 = rev l2.
    Proof. 
      intros l1 l2.
...
Theorem beq_id_n_n : forall n:id, true = beq_id n n.
Proof.
  intros n.
  destruct n as [n'].
  simpl.
...
(** How _many_ elements does the type [baz] have?
    ...
    0 There is no single constructor of baz*)
